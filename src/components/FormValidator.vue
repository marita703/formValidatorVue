...
const submitForm = () => {
  if (isFormValid.value) {
    console.log('Form is submited', formData.value)
  } else {
  ...
  <div>
    <form @submit.prevent="submitForm" class="custom-form">
      ...
      <div class="form-group">
        ...
        <span v-if="!isNameEmail" class="error">Please enter a valid email address</span>
      </div>
      <div class="form-group">
        <label for="password">Password</label>
        <input type="text" v-model="formData.password" id="password" />
        <span v-if="!isPasswordValid" class="error">Password must be at least 8 caracthers</span>
      </div>
